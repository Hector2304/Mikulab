...
class ReservacionDAO extends AbstractDAO implements IReservacionDAO
{
    ...
    public function cancelar(int $idReservacion, int $idProfesor = null): bool
    {
        try {
            $conn = $this->conexion->getConexion();
            $connExterna = LaboratoriosFCABD::getInstance()->getConexion(); // Conexión a BD externa
...
//Funcion mejorada de la anterior, ni idea si jala chido
    public function consultarClasesPredeterminadasPorSemana($year, $week, $labId = null, $verificarReservas = true)
    {
        ...
        ");
            ...
            $this->logDepuracion("🔢 Clases predeterminadas detectadas para semana {$week} de {$year}: " . count($clasesPosibles));
    ...
    private function logDepuracion($mensaje, $archivo = "clases_predeterminadas_debug.txt")
    {
