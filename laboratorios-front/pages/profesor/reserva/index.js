...
export default {
    ...
    numHorasSeleccionables() {
      const grupo = this.grupoSelected;

      // 🔍 Obtener la clave del grupo según el tipo
      const claveGrupo =
        grupo?.grupClave || grupo?.gruoClave || grupo?.grotClave || "";

      // ✅ Si empieza con EA → 3 horas
      if (claveGrupo.startsWith("EA")) {
        return 3;
      }

      // ✅ Lógica por tipo de grupo
      if (grupo?.grupIdGrupo) return 2;     // Licenciatura
      if (grupo?.gruoIdGrupoP) return 3;    // Posgrado
      if (grupo?.grotIdGrupoOt) return 5;   // Titulación
  ...
  async mounted() {
    this.cargandoTodo = true;
    this.loadGrupos();
    this.loadLaboratorios();
    this.loadSoftwares();
    this.loadAsocLabSoft();
    this.cargandoTodo = false;
  },
...
,
    ...
    async loadDisponibilidad(newDate) {
      if (this.disponibilidadLoading) return;

      const fechaSeleccionada = this.$moment(newDate, 'YYYY-MM-DD');


      // ⏳ Cargar clases predeterminadas solo del día seleccionado
      await this.loadClasesPredeterminadasPorDia(fechaSeleccionada.format('YYYY-MM-DD'));
      ...
      const diaSemana = fechaSeleccionada.day(); // 0=domingo...6=sábado
      this.isSabado = diaSemana === 6;

      const labIds = this.laboratorios.map((lab) => lab.saloIdSalon);
      const hrsNoDisp = {};
      ...
      try {
        ...
        for (let horario of response.data) {
          const idStr = `${horario.labId}`;
          if (!hrsNoDisp[idStr]) hrsNoDisp[idStr] = new Set();
      ...
      // 🔹 Cursos externos
      try {
        const response = await this.$axios.post('controller/externo/curso/asignacion-horario-get-disponibilidad.php', {
          fecha: newDate,
          labIds
        });
      ...
      const lunesSemana = fechaSeleccionada.clone().startOf('isoWeek');
      ...
      for (let clase of this.clasesPredeterminadas) {
        ...
        if (!hrsNoDisp[labId]) hrsNoDisp[labId] = new Set();
    ...
    async loadClasesPredeterminadasPorDia(fechaISO) {
      try {
        const response = await this.$axios.post(
          'http://localhost:7070/api/controller/reserva/predeterminadasPorDia.php',
          { fecha: fechaISO },
          {
        ...
        console.log('🧠 Clases predeterminadas para el día:', fechaISO, this.clasesPredeterminadas);
      } catch (error) {
