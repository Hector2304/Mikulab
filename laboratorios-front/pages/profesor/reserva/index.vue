      ...
      <b-collapse v-model="laboratoriosVisibles">
        ...
        >
          ...
          <template v-slot:horario="{ item }">
            <b-skeleton-icon
              class="w-100"
              v-if="disponibilidadLoading || !disponibilidad"
              :animation="disponibilidadLoading ? 'wave' : ''"
              :icon="disponibilidadLoading ? 'clock-fill' : 'clock'"
              :icon-props="{ fontScale: 1.75 }"
            ></b-skeleton-icon>
            <ReservaHoraDia
              v-if="disponibilidad && !disponibilidadLoading"
